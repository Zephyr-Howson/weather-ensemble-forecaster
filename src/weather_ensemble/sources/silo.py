...
def fetch_actual(location: Location, target_date: date) -> ActualRecord:
    ...
    collection pipeline (service.record_actual/backfill): the actuals table allows
    multiple sources per day, but load_modelling_table's forecast-to-actual join
    assumes exactly one actuals row per (location, date) - adding a second source
    there would silently duplicate every forecast row in the join. Call this
    directly if you want to compare or switch to it deliberately.
    """
